...
exports.PostUserRegister = async (req, res) => {
  ...
  if (!password && password.length < 8) {
    return res.json({
      status: "password Must be greater than 8 words",
      Ok: "false",
    });
  }
...
exports.PostLogin = async (req, res) => {
  ...
  if(userfind.isLogin === true)
  {
      return res.json({ status: "ok", result: "already logged in" });
  }
...
exports.PostLogOut = async (req, res) => {
  ...
  const userfind = await user.findOne({ email });
  const _id = userfind._id;
  if (userfind.isLogin === false) {
    return res.json({ status: "ok", result: "already logged out" });
  }
  const userupdate = user.findByIdAndUpdate(
    _id,
    { isLogin: false },
